...
def getRep(imgPath, multiple=False):
    ...
    for bb in bbs:
        ...
        alignedFace = align.align(
            ...
            rgbImg,
            bb,
            landmarkIndices=facelib.AlignDlib.OUTER_EYES_AND_NOSE)
...
""")
    ...
    net = facelib.TorchNeuralNet(args.networkModel, imgDim=args.imgDim,
                                  cuda=args.cuda)

    if args.verbose:
        print("Loading models took {} seconds.".format(
            time.time() - start))
        start = time.time()
